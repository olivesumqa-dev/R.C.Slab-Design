...
export default function RCSlabCalculator({ onClose, title = "Reinforced Concrete Slab Design" }: Props) {
  ...
  const exportBOQ = () => {
    ...
    const dim = r.effective_type === "One-Way"
      ...
      : `${(r.inputs.Lx/1000).toFixed(2)}×${(r.inputs.Ly/1000).toFixed(2)} m, ${r.inputs.h} mm thick`;
    const rows = [
      ["#", "Description", "Specification", "Quantity", "Unit"],
      ["1", "Concrete, ready-mix (+10% waste)", `f'c = ${r.inputs.fc} MPa, ${dim}`, r.qty.conc_order.toFixed(3), "m³"],
      ...(r.effective_type === "One-Way"
        ? [
            ["2", "Main rebar (bottom)", `φ${r.inputs.db_1w}mm @ ${r.oneway!.s_adopted}mm o.c.`, r.qty.steel_short.toFixed(1), "kg"],
            ["3", "Temperature rebar", `φ${r.inputs.dt_1w}mm @ ${r.oneway!.s_temp_adopted}mm o.c.`, r.qty.steel_long.toFixed(1), "kg"],
          ]
        : [
            ["2", "Rebar short direction", `φ${r.inputs.db_short}mm @ ${r.twoway!.s_short_pos}mm o.c.`, r.qty.steel_short.toFixed(1), "kg"],
            ["3", "Rebar long direction", `φ${r.inputs.db_long}mm @ ${r.twoway!.s_long_pos}mm o.c.`, r.qty.steel_long.toFixed(1), "kg"],
          ]),
      ["4", "Total reinforcement (incl. 10% lap)", `fy = ${r.inputs.fy} MPa`, r.qty.steel_total.toFixed(1), "kg"],
      ["5", "Formwork", r.effective_type === "One-Way" ? "soffit per m width" : "soffit + edges", r.qty.formwork.toFixed(2), "m²"],
    ];
        ...
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            ...
            <p className="font-sans text-[10px] tracking-widest uppercase" style={{ color: "#a8c5e8" }}>
              {currentName ? `${currentName}${currentFolder ? ` · ${currentFolder}` : ""}` : `NSCP 2015 / ACI 318-19 · ${results.effective_type}`}
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[440px_1fr] gap-0 min-h-[calc(100vh-65px)]">
        ...
        <aside style={{ background: CARD_BG, borderRight: `1px solid ${RULE_LIGHT}` }} className="p-6 lg:p-8 no-print">
          ...
          <div className="font-sans text-[10px] italic py-1.5" style={{ color: MUTED }}>
            Auto-detect uses Ly/Lx threshold: &gt;2 → one-way; ≤2 → two-way.
          </div>
          ...
          {(results.effective_type === "One-Way" || inputs.slabType === "One-Way") && (
            <>
              ...
              <Field label="Main bar Ø, db" unit="mm" value={inputs.db_1w} onChange={v => setField("db_1w", +v)} />
              <Field label="Temp bar Ø, dt" unit="mm" value={inputs.dt_1w} onChange={v => setField("dt_1w", +v)} />
            </>
          )}
          ...
          {(results.effective_type === "Two-Way" || inputs.slabType === "Two-Way") && (
            <>
              ...
              <Field label="Bar Ø short dir, db_x" unit="mm" value={inputs.db_short} onChange={v => setField("db_short", +v)} />
              <Field label="Bar Ø long dir, db_y" unit="mm" value={inputs.db_long} onChange={v => setField("db_long", +v)} />
          ...
          <p className="font-sans text-[10px] leading-relaxed mt-6 pt-4"
            ...
            Self-weight is calculated automatically from h × γc. Edge case selection uses full ACI Method 3 (9 cases). Coefficient interpolation is segment-by-segment linear (not regression).
          </p>
        </aside>
        ...
        <main style={{ background: PAPER_BG }} className="p-6 lg:p-10 print-area">
          ...
          <ResultBlock title="1. Loads & Combinations (NSCP/ACI)">
            <Row label="Self-weight (auto from h × γc)" val={fmt(inputs.h / 1000 * inputs.gamma_c, 2)} unit="kPa" />
          ...
          <ResultBlock title="2. Material Properties (Derived)">
            <Row label="Ec = 4700·√f'c" val={fmt(results.Ec, 1)} unit="MPa" />
            <Row label="fr = 0.62·√f'c" val={fmt(results.fr, 3)} unit="MPa" />
            <Row label="β₁" val={fmt(results.beta1, 3)} />
            <Row label="ρ_min = max(0.25√f'c/fy, 1.4/fy)" val={fmt(results.rho_min, 5)} />
            <Row label="ρ_max (tension-controlled, εt ≥ 0.005)" val={fmt(results.rho_max, 5)} />
          ...
          {results.effective_type === "One-Way" && results.oneway && (
            <>
              <ResultBlock title="3. Analysis — Moment & Shear (ACI 318-19 §6.5)">
                <Row label="Effective depth, d = h − cc − db/2" val={fmt(results.oneway.d, 1)} unit="mm" />
                <Row label="Moment coefficient, Cm" val={fmt(results.oneway.Cm, 4)}
                  formula={`${inputs.support_1w}`} />
                <Row label="Factored moment, Mu" val={fmt(results.oneway.Mu, 3)} unit="kN·m" />
                <Row label="Factored shear, Vu" val={fmt(results.oneway.Vu, 3)} unit="kN" />
              </ResultBlock>

              <ResultBlock title="4. Flexural Design (Bottom Bars)">
                <Row label="Rn = Mu / (φ·b·d²)" val={fmt(results.oneway.Rn, 4)} unit="MPa" />
                <Row label="Required ρ" val={fmt(results.oneway.rho_req, 5)} />
                <Row label="Design ρ = max(ρ_req, ρ_min)" val={fmt(results.oneway.rho_design, 5)} />
                <Row label="As required" val={fmt(results.oneway.As_req, 1)} unit="mm²/m" />
                <Row label="Bar area Ab (φ${inputs.db_1w})"
                  val={fmt(results.oneway.Ab, 1)} unit="mm²" />
                ...
                <Row label="Adopted spacing (↓25mm)" val={fmt(results.oneway.s_adopted, 0)} unit="mm" bold />
                <Row label="As provided" val={fmt(results.oneway.As_prov, 1)} unit="mm²/m" />
                <Row label="ρ provided" val={fmt(results.oneway.rho_prov, 5)} />
                <Row label="ρ_max check (≤ ρ_max)" val={results.oneway.rho_max_status} />
              ...
              <ResultBlock title="5. Temperature & Shrinkage Reinforcement (ACI 9.6.4)">
                <Row label="ρ_temp" val={fmt(inputs.rho_temp, 5)} />
                <Row label="As_temp required = ρ_temp × b × h" val={fmt(results.oneway.As_temp_req, 1)} unit="mm²/m" />
                <Row label={`Temp bar area (φ${inputs.dt_1w})`} val={fmt(results.oneway.Ab_temp, 1)} unit="mm²" />
              ...
              <ResultBlock title="6. Shear Check (ACI 22.5)">
                <Row label="φVc = φ·0.17·λ·√f'c·b·d" val={fmt(results.oneway.phi_Vc, 3)} unit="kN" />
                <Row label="Vu (demand)" val={fmt(results.oneway.Vu, 3)} unit="kN" />
                <Row label="Utilization Vu / φVc" val={fmt(results.oneway.shear_util, 4)} />
              ...
              <ResultBlock title="7. Deflection Check (ACI 9.3 / 24.2)">
                ...
                <Row label="Ig = b·h³/12" val={fmt(results.oneway.Ig, 0)} unit="mm⁴" />
                <Row label="Immediate deflection, Δᵢ" val={fmt(results.oneway.delta_immediate, 3)} unit="mm" />
                <Row label="Long-term deflection (ξ=2.0, ACI 24.2.4)" val={fmt(results.oneway.delta_long_term, 3)} unit="mm" />
          ...
          {results.effective_type === "Two-Way" && results.twoway && (
            <>
              <ResultBlock title="3. Two-Way Analysis (ACI Method 3)">
                ...
                <Row label="Bar area Ab short" val={fmt(results.twoway.Ab_short, 1)} unit="mm²" />
                <Row label="Bar area Ab long" val={fmt(results.twoway.Ab_long, 1)} unit="mm²" />
              </ResultBlock>
              ...
              <ResultBlock title="5. Factored Moments">
                <Row label="Ma_DL = Ca_DL × wDL × Lx²" val={fmt(results.twoway.Ma_DL, 3)} unit="kN·m/m" />
                <Row label="Ma_LL = Ca_LL × wLL × Lx²" val={fmt(results.twoway.Ma_LL, 3)} unit="kN·m/m" />
                <Row label="Mb_DL = Cb_DL × wDL × Ly²" val={fmt(results.twoway.Mb_DL, 3)} unit="kN·m/m" />
                <Row label="Mb_LL = Cb_LL × wLL × Ly²" val={fmt(results.twoway.Mb_LL, 3)} unit="kN·m/m" />
                <Row label="Mu_short (positive) = 1.2Ma_DL + 1.6Ma_LL" val={fmt(results.twoway.Mu_short_pos, 3)} unit="kN·m/m" bold />
                <Row label="Mu_long (positive)" val={fmt(results.twoway.Mu_long_pos, 3)} unit="kN·m/m" bold />
                <Row label="Mu_short (negative at supports)" val={fmt(results.twoway.Mu_short_neg, 3)} unit="kN·m/m" />
                <Row label="Mu_long (negative at supports)" val={fmt(results.twoway.Mu_long_neg, 3)} unit="kN·m/m" />
              </ResultBlock>
              ...
              <ResultBlock title="6. Flexural Design — Bottom Bars (Midspan)">
                ...
                <Row label="As required" val={fmt(results.twoway.As_short_pos, 1)} unit="mm²/m" />
                <Row label="Adopted spacing" val={`φ${inputs.db_short} @ ${results.twoway.s_short_pos}`} unit="mm" bold />
                <Row label="As provided" val={fmt(results.twoway.As_short_pos_prov, 1)} unit="mm²/m" />
                ...
                <Row label="As required" val={fmt(results.twoway.As_long_pos, 1)} unit="mm²/m" />
                <Row label="Adopted spacing" val={`φ${inputs.db_long} @ ${results.twoway.s_long_pos}`} unit="mm" bold />
                <Row label="As provided" val={fmt(results.twoway.As_long_pos_prov, 1)} unit="mm²/m" />
              </ResultBlock>
              ...
              {(results.twoway.Mu_short_neg > 0 || results.twoway.Mu_long_neg > 0) && (
                <ResultBlock title="7. Flexural Design — Top Bars (Negative Moment at Continuous Supports)">
                  ...
                  <Row label="As required (top)" val={fmt(results.twoway.As_short_neg, 1)} unit="mm²/m" />
                  <Row label="Adopted spacing" val={`φ${inputs.db_short} @ ${results.twoway.s_short_neg}`} unit="mm" bold />
                  <Row label="As provided" val={fmt(results.twoway.As_short_neg_prov, 1)} unit="mm²/m" />
                  <h4 className="font-sans text-[11px] tracking-widest uppercase mb-2 mt-4" style={{ color: ACCENT_DARK }}>
                    Long direction
                  </h4>
                  <Row label="As required (top)" val={fmt(results.twoway.As_long_neg, 1)} unit="mm²/m" />
                  <Row label="Adopted spacing" val={`φ${inputs.db_long} @ ${results.twoway.s_long_neg}`} unit="mm" bold />
                  <Row label="As provided" val={fmt(results.twoway.As_long_neg_prov, 1)} unit="mm²/m" />
                </ResultBlock>
              )}

              <ResultBlock title="8. One-Way Shear Check (ACI 22.5)">
                <Row label="Vu = wu × (Lx/2 − dx)" val={fmt(results.twoway.Vu_oneway, 3)} unit="kN/m" />
                <Row label="φVc (per m width)" val={fmt(results.twoway.phi_Vc_oneway, 3)} unit="kN/m" />
                <Row label="One-way shear status" val={results.twoway.oneway_status} bold />
              </ResultBlock>

              <ResultBlock title="9. Punching Shear Check (ACI 22.6)">
                <Row label="Column dimensions, c1 × c2" val={`${inputs.c1} × ${inputs.c2}`} unit="mm" />
                <Row label="β_col = long / short" val={fmt(results.twoway.beta_col, 3)} />
                ...
                <Row label="Vc1 = 0.33·λ·√f'c·bo·d" val={fmt(results.twoway.Vc1, 3)} unit="kN" />
                <Row label="Vc2 = 0.17·(1+2/β)·λ·√f'c·bo·d" val={fmt(results.twoway.Vc2, 3)} unit="kN" />
                <Row label="Vc3 = 0.083·(αs·d/bo+2)·λ·√f'c·bo·d" val={fmt(results.twoway.Vc3, 3)} unit="kN" />
                <Row label="φVc punch (min of 3, × φ)" val={fmt(results.twoway.phi_Vc_punch, 3)} unit="kN" />
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody className="font-sans text-xs">
                <BOQRow n="1" desc="Concrete, ready-mix (+10% waste)"
                  spec={`f'c = ${inputs.fc} MPa`} qty={fmt(results.qty.conc_order, 3)} unit="m³" />
                {results.effective_type === "One-Way" ? <>
                  <BOQRow n="2" desc="Main rebar (bottom)"
                    spec={`φ${inputs.db_1w}mm @ ${results.oneway!.s_adopted}mm o.c.`}
                    qty={fmt(results.qty.steel_short, 1)} unit="kg" />
                  <BOQRow n="3" desc="Temperature rebar"
                    spec={`φ${inputs.dt_1w}mm @ ${results.oneway!.s_temp_adopted}mm o.c.`}
                    qty={fmt(results.qty.steel_long, 1)} unit="kg" />
                </> : <>
                  <BOQRow n="2" desc="Rebar short direction"
                    spec={`φ${inputs.db_short}mm @ ${results.twoway!.s_short_pos}mm o.c.`}
                    qty={fmt(results.qty.steel_short, 1)} unit="kg" />
                  <BOQRow n="3" desc="Rebar long direction"
                    spec={`φ${inputs.db_long}mm @ ${results.twoway!.s_long_pos}mm o.c.`}
                    qty={fmt(results.qty.steel_long, 1)} unit="kg" />
                </>}
                ...
                <BOQRow n="5" desc="Formwork"
                  ...
                  qty={fmt(results.qty.formwork, 2)} unit="m²" />
              </tbody>
            </table>
          </ResultBlock>
          ...
          <ResultBlock title={`${results.effective_type === "One-Way" ? "10" : "12"}. Design Summary & Certification`}>
            ...
            <Row label="Geometry" val={results.effective_type === "One-Way"
              ? `${(inputs.Ln_1w/1000).toFixed(2)} m × ${inputs.h} mm thick`
              : `${(inputs.Lx/1000).toFixed(2)} × ${(inputs.Ly/1000).toFixed(2)} × ${inputs.h} mm`} />
            <Row label="Governing wu" val={`${fmt(results.loads.wu, 2)} kPa (${results.loads.governing})`} />
            {results.effective_type === "One-Way" && results.oneway && <>
              <Row label="Main bars (bottom)" val={`φ${inputs.db_1w}mm @ ${results.oneway.s_adopted}mm o.c.`} />
              <Row label="Temperature bars" val={`φ${inputs.dt_1w}mm @ ${results.oneway.s_temp_adopted}mm o.c.`} />
              <Row label="Flexure" val={<StatusPill status={results.oneway.flex_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
              <Row label="Shear" val={<StatusPill status={results.oneway.shear_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
              <Row label="Deflection" val={<StatusPill status={results.oneway.defl_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
              <Row label="Thickness" val={<StatusPill status={results.oneway.thickness_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
              <Row label="Crack control" val={<StatusPill status={results.oneway.crack_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
            </>}
            {results.effective_type === "Two-Way" && results.twoway && <>
              <Row label="Bars (short dir)" val={`φ${inputs.db_short}mm @ ${results.twoway.s_short_pos}mm`} />
              <Row label="Bars (long dir)" val={`φ${inputs.db_long}mm @ ${results.twoway.s_long_pos}mm`} />
              <Row label="One-way shear" val={<StatusPill status={results.twoway.oneway_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
              <Row label="Punching shear" val={<StatusPill status={results.twoway.punch_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
              <Row label="Thickness" val={<StatusPill status={results.twoway.thickness_status.startsWith("✓") ? "PASS" : "FAIL"} />} />
            </>}
            ...
            <p className="font-sans text-[11px] italic leading-relaxed mt-4" style={{ color: MUTED }}>
              ...
              (ξ = 2.0 for sustained loads &gt; 5 years).
            </p>
          </ResultBlock>
...
function SlabSVG({ results }: { results: RCSlabResults }) {
    ...
    return (
      <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-2xl mx-auto" xmlns="http://www.w3.org/2000/svg">
        ...
        <text x={W/2} y={slab_top - 12} textAnchor="middle" fontFamily="sans-serif" fontSize="11" fill="#c0392b">
          ↑ Temp bars φ{i.dt_1w}mm @ {results.oneway!.s_temp_adopted}mm o.c. (top)
        </text>
        <text x={W/2} y={slab_bot + 15} textAnchor="middle" fontFamily="sans-serif" fontSize="11" fill={ACCENT_DARK}>
          ↓ Main bars φ{i.db_1w}mm @ {results.oneway!.s_adopted}mm o.c. (bottom)
        </text>
      </svg>
    );
  }
  ...
  // Short direction bars (parallel to Lx → vertical in our orientation)
  ...
  return (
    <svg viewBox={`0 0 ${W} ${H}`} className="w-full max-w-md mx-auto" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x={W/2} y={y0 + slab_h + 28} textAnchor="middle" fontFamily="sans-serif" fontSize="11" fill={ACCENT}>
        Bottom bars short dir: φ{i.db_short}mm @ {results.twoway!.s_short_pos}mm o.c.
      </text>
      <text x={W/2} y={y0 + slab_h + 44} textAnchor="middle" fontFamily="sans-serif" fontSize="11" fill="#c0392b">
        Bottom bars long dir: φ{i.db_long}mm @ {results.twoway!.s_long_pos}mm o.c.
      </text>
